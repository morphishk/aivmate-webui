...
// 修复 Three.js 与 MMDLoader.js 版本兼容警告：MeshToonMaterial 不再支持 envMap/combine
if (typeof THREE !== 'undefined' && THREE.MeshToonMaterial && THREE.MeshToonMaterial.prototype.setValues) {
    const _originalSetValues = THREE.MeshToonMaterial.prototype.setValues;
    THREE.MeshToonMaterial.prototype.setValues = function(values) {
        if (values) {
            var cleaned = {};
            for (var key in values) {
                if (values.hasOwnProperty(key) && key !== 'envMap' && key !== 'combine') {
                    cleaned[key] = values[key];
                }
            }
            _originalSetValues.call(this, cleaned);
        } else {
            _originalSetValues.call(this, values);
        }
    };
}
...
function init() {
    ...
    // 创建渲染器: alpha通道让CSS背景显示; 移除scene.background避免背景拉伸
